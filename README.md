...
Network settings use Add/Edit dialogs. Enter only an RPC URL; chain ID is detected with eth_chainId. Saved legacy contract overrides are replaced with the bundled addresses. Settings import/export and manual contract verification controls are removed; transaction and deployment verification still run automatically. The account control uses a local deterministic identicon, and creation starts from the Positions page. IPFS distribution information remains in these docs and CI links, not in the app.
