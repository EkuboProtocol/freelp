...
The application retains only EVM LP management: token selection/import, pool selection, current liquidity graphs, position creation/add/remove/collect/transfer/burn, per-network RPC configurations, and browser contract deployment. The four default mainnets and optional viem networks are listed in the README. No indexer, server, hosted token list, Starknet, swap, or TWAMM feature is required.
