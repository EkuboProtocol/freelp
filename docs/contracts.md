...
Writes use flat arguments: `maybeInitializePool(key, initialTick)`, `createPosition(key, lower, upper, maxAmount0, maxAmount1, minLiquidity)`, `addLiquidity(id, maxAmount0, maxAmount1, minLiquidity)`, and `withdraw(id, liquidity, recipient)`. Creation and additions require positive minimum liquidity. Withdrawals have no minimum-output arguments; the UI displays estimated receipts and makes this explicit, with slippage controls limited to deposits. The UI retains fresh previews, simulation, exact token caps, wallet identity checks, and submitted-request recovery.
