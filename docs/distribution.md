...
After npm publication, recommend `bunx @ekubo/freelp` or `npx @ekubo/freelp`. Both serve the application bundled in the installed package and open a browser. Node 22+ supports the npx path; users do not need Bun to run it. Pin a version with `@ekubo/freelp@0.1.1`. To pass options explicitly with npx: `npx -- @ekubo/freelp --no-browser --port 4173`.
