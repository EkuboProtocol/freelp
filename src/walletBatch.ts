...
export async function executeBatch(
  ...
  if ((await client.getChainId()) !== settings.chainId)
    throw new Error(t`RPC chain ID does not match settings.`);
  ...
  const batch = calls.map((call) => {
    if (!call.to) throw new Error(t`Batch calls require a destination.`);
  ...
  const simulation = await client
    ...
    .catch(() => {
      throw new Error(
        t`Unable to simulate this batch. Approve the tokens separately, then deposit.`,
      );
    });
  if (simulation.results.some((result) => result.status !== "success"))
    throw new Error(
      t`The batch simulation failed. Review the deposit amounts and approvals.`,
    );
  await assertWallet(provider, account, settings.chainId);
  ...
  if (!last)
    throw new Error(
      t`The wallet has not returned a transaction receipt. Check the wallet before retrying.`,
    );
  const receipt = await client.waitForTransactionReceipt({
    hash: last.transactionHash,
  });
  if (receipt.status !== "success")
    throw new Error(t`The deposit reverted. Refresh balances before retrying.`);
  return receipt;
}
