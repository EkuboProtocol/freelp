...
.cache/bin/ipfs --offline dag import release/site.car
...
if [[ "${FREELP_PUBLIC_RELEASE:-false}" == true ]]; then
  ...
  curl --fail --silent --show-error -X POST "$FREELP_PIN_API/api/v0/dag/import?pin-roots=true" -F file=@release/site.car
fi
